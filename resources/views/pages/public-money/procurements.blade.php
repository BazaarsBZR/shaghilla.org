            ...
            @forelse ($records as $record)
                <article class="p-5">
                    <div class="flex items-center justify-between gap-3">
                        <span class="rounded-full bg-[#e7f3ec] px-3 py-1 text-[11px] font-black text-[#21704f]">{{ $record->publicMoneyStageLabel() }}</span>
                    ...
                    <dl class="mt-4 grid grid-cols-2 gap-3 rounded-2xl bg-surface-soft p-4 text-sm">
                        <div><dt class="text-xs font-bold text-ink-muted">{{ __('ui.public_money.supplier') }}</dt><dd class="mt-1 font-black text-ink">{{ $record->supplierDisplayLabel() }}</dd></div>
                        <div><dt class="text-xs font-bold text-ink-muted">{{ __('ui.public_money.amount') }}</dt><dd class="mt-1 font-black text-ink" dir="auto">{{ $record->amountDisplayLabel() }}</dd></div>
                    </dl>
                </article>
            @empty
