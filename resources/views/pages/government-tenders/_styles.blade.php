...
.gt-wrap { width: min(1180px, calc(100% - 32px)); margin-inline: auto; }
.gt-hero {
    position: relative;
    overflow: hidden;
    padding: 64px 0 96px;
    color: white;
    background:
        linear-gradient(115deg, rgba(5, 45, 54, .98), rgba(9, 92, 71, .92)),
...
.gt-hero h1 { max-width: 760px; margin: 0; font-size: clamp(2rem, 5vw, 4.2rem); line-height: 1.15; }
...
.gt-counter {
    ...
    box-shadow: 0 16px 46px rgba(16,41,51,.09);
}
.gt-counter span { display: block; color: var(--gt-muted); font-weight: 700; }
.gt-counter strong { display: block; margin-top: 7px; color: var(--gt-green); font-size: 2rem; }
...
.gt-filter-panel {
    ...
    box-shadow: 0 10px 30px rgba(16,41,51,.05);
}
...
.gt-card {
    ...
    border-radius: 22px;
    background: var(--gt-card);
    box-shadow: 0 10px 28px rgba(16,41,51,.045);
}
...
.gt-pagination { display: flex; align-items: center; justify-content: center; gap: 14px; margin-top: 30px; }
.gt-pagination a { color: var(--gt-green); font-weight: 800; text-decoration: none; }
...
.gt-panel { padding: 26px; border: 1px solid var(--gt-line); border-radius: 22px; background: white; box-shadow: 0 12px 36px rgba(16,41,51,.07); }
