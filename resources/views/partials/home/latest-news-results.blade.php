...
@if (isset($latestPaginator) && $latestPaginator->lastPage() > 1)
    ...
    <nav class="flex flex-wrap items-center justify-center gap-2 pt-2" aria-label="Latest news pages">
        @if ($currentPage > 1)
            <a
        ...
        @for ($page = 1; $page <= $lastPage; $page++)
            <a
                href="{{ route('home', ['latest_page' => $page]) }}#latest-news"
                data-latest-page="{{ $page }}"
                @class([
                    'inline-flex h-9 w-9 items-center justify-center rounded-control border text-sm font-extrabold transition',
                    'border-night bg-night text-white' => $page === $currentPage,
                    'border-line bg-surface text-ink-muted hover:bg-surface-soft hover:text-ink' => $page !== $currentPage,
                ])
